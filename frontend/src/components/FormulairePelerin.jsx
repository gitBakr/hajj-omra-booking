    ...
    try {
      ...
      const resultats = await Promise.all(
        formulaires.map(async (form) => {
          ...
          if (!response.ok) {
            throw new Error(
              `Un(e) pèlerin(e) nommé(e) ${form.data.prenom} ${form.data.nom} est déjà enregistré(e). ` +
              `Pour plus d'informations, appelez le 07 83 09 08 15`
            );
          }
